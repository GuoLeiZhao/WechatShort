...
class SDK {
	...
	_postToEQq(body) {
      ...
      const url = 'https://your-admin-domain.com/sqx_fast/app/user/action/add';
      // const url = 'http://localhost:8183/sqx_fast/app/user/action/add';
			const timestampSec = Math.floor(Date.now() / 1000);
			const headers = {
	...
	_getClickIdFromStorage() {
		try {
			if (typeof wx !== 'undefined' && wx.getStorageSync) {
				const v = wx.getStorageSync('gdt_vid');
				return v || '';
			}
		} catch (e) {}
		return '';
