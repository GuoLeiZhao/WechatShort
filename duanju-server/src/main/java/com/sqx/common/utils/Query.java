...
public class Query<T> {
    ...
    public IPage<T> getPage(BasePagingRequest pageParam){
        Page<T> tPage = new Page<>(pageParam.getCurrPage(), pageParam.getPageSize());
        if (CollUtil.isNotEmpty(pageParam.getOrders())) {
            List<OrderItem> orderItems = pageParam.getOrders().stream().map(item -> {
                OrderItem orderItem = new OrderItem();
                orderItem.setColumn(item.getOrder());
